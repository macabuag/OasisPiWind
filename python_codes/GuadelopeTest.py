...
vulnerability = Path.cwd() / 'inputs'/ 'vulnerability' / 'GLP_Vulnerability.csv'
#vulnerability = Path.cwd() / 'inputs'/ 'vulnerability' / 'GLP_1.csv'
...
fp_combine_1 = Path.cwd() / 'outputs' / 'csv' / 'combined_wind_exposure.csv'
fp_groupby_1 = Path.cwd() / 'outputs' / 'csv' / 'groupby_wind_exposure.csv'
fp_groupby_2 = Path.cwd() / 'outputs' / 'csv' / 'groupby_wind_exposure_intensity.csv'
...
def plot_multi_vf(path_in, path_out_df):
    ...
    for file in files:
        ...
        df_val_vf.to_csv( file.csv , index=False)
...
def main():
    ...
    # load the data
    df_event = wnf.load_data_IBTrACS(ibtracs)
    #print(df_event.info())
    df_event['V'] = df_event['Vmax_ms']
    df_event.loc[:,'category'] = df_event.apply(wnf.get_wind_category,axis=1)
    
    #trasform it to a geodataframe
    gdf_event = wnf.geolocalization(df_event)
    
    #restimate Rmax (the selected windtrack does not have it)
    gdf_event['RMW_km'] = wnf.Rmax_W06(gdf_event['Vmax_ms'], gdf_event['Lat'])
    #print(gdf_event.info())
    ...
    glp_vf = load_vf(vulnerability)
    vf = save_as_single_vf(glp_vf)
    glp_vf_merge = plot_multi_vf(path_in, path_out_df)
    
    #vulnerability_dictionary = vulnerability_dic(glp_vf_1, OccupancyCode = 1102 ) #change this accordinly to the exposure
    vulnerability_dictionary = vulnerability_dic(glp_vf_merge, OccupancyCode = 1102 )
    ...
    df_lookup_2 = damage_intensity_lookup(glp_vf_merge, df_damage, df_intensity)
